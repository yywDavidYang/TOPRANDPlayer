...
{        
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, _framebuffer);//绑定_framebuffer，OpenGL里面有许多的类似的Bind函数，之后所有对frameBuffer的操作就是针对这个frameBuffer的
    ...
    if (frame) {
        [_renderer setFrame:frame];// 就是OpenGL的绘制结果不是直接显示到屏幕上，而是存起来了，这个存储的东西就是FBO
    }
